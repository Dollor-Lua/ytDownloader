...
download.onclick = async function(e) {
    ...
    if (CheckYTUrl(video.value)) {
        ...
        if (selection.value == "mp3") {
            ytdl(video.value, {filter: 'audioonly'}).pipe(fs.createWriteStream(`${pathName}.mp3`))
        } else if (selection.value == "mp4") {
            ytdl(video.value).pipe(fs.createWriteStream(`${pathName}.mp4`))
        }
    }
}